...
# la cache no expira; vive lo que viva el proceso. La usa también /consulta/comprobantes.
...
def texto(d_id: int, request: Request, db: Session = Depends(get_db),
    ...
    """Texto extraído del documento, equipo-only (mismos roles que la consulta analítica:
    el propietario ya tiene su vía de acceso al archivo citado vía /contenido)."""
    d = db.get(models.Documento, d_id)
    if not d:
        raise HTTPException(404, "No existe ese documento")
    t = extraer_texto(request.app.state.storage, d)
