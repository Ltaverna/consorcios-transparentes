...
- [ ] **B5. API arriba:** `docker compose build` → `docker compose run --rm api alembic upgrade head` (migra ANTES de arrancar: el `create_all` del startup crearía las tablas y el upgrade posterior fallaría con DuplicateTable) → `docker compose up -d` → `curl -s localhost:8080/salud` → `{"ok":true}`. (Si el build falla, el sospechoso es el empaquetado — ver nota final de DEPLOY.md.)
- [ ] **B6. Datos iniciales:** `docker compose exec api python cli.py init "Rivadavia 2069" --direccion "Av. Rivadavia 2069, CABA"` → `docker compose exec -it api python cli.py usuario <email-real> "Lucas" auditor`.
